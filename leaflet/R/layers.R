...
# addRegPolygonMarkers
#' @export
addRegPolygonMarkers = function(
  map, lng = NULL, lat = NULL, layerId = NULL, group = NULL,
  stroke = TRUE,
  color = "#03F",
  radius = 5,
  ...
  options = c(options, list(
    stroke = stroke, color = color, radius = radius, rotation = rotation, numberOfSides = numberOfSides, opacity = opacity,
    fill = fill, fillColor = fillColor, fillOpacity = fillOpacity,
    dashArray = dashArray
  ))

  map$dependencies = c(map$dependencies, markerDvfDependencies())

  if (!is.null(clusterOptions))
    map$dependencies = c(map$dependencies, markerClusterDependencies())

  pts = derivePoints(data, lng, lat, missing(lng), missing(lat), "addRegPolygonMarkers")
  invokeMethod(map, data, 'addRegPolygonMarkers', pts$lat, pts$lng,
...
addStarMarkers = function(
  ...
  options = c(options, list(
    stroke = stroke, color = color, maxDegrees = maxDegrees, rotation = rotation, numberOfPoints = numberOfPoints, opacity = opacity,
    fill = fill, fillColor = fillColor, fillOpacity = fillOpacity, gradient = gradient, dropShadow = dropShadow,
    dashArray = dashArray
  ))
  ...
  pts = derivePoints(data, lng, lat, missing(lng), missing(lat), "addStarMarkers")
  invokeMethod(map, data, 'addStarMarkers', pts$lat, pts$lng,
               layerId, group, options, clusterOptions, clusterId, popup) %>%
    expandLimits(pts$lat, pts$lng)
}
...
addDiamondMarkers = function(
  ...
  options = c(options, list(
    stroke = stroke, color = color, radiusX = radiusX, radiusY = radiusY, rotation = rotation, numberOfSides = numberOfSides, opacity = opacity,
    fill = fill, fillColor = fillColor, fillOpacity = fillOpacity,
    dashArray = dashArray
  ))
  ...
  pts = derivePoints(data, lng, lat, missing(lng), missing(lat), "addDiamondMarkers")
  invokeMethod(map, data, 'addDiamondMarkers', pts$lat, pts$lng,
               layerId, group, options, clusterOptions, clusterId, popup) %>%
    expandLimits(pts$lat, pts$lng)
}
...
addPieChartMarkers = function(
  ...
  options = c(options, list(
    stroke = stroke, color = color, weight = weight, radius = radius, rotation = rotation, numberOfSides = numberOfSides, opacity = opacity,
    fill = fill, fillColor = fillColor, fillOpacity = fillOpacity, mouseOverExaggeration = mouseOverExaggeration,
    dashArray = dashArray, maxDegrees = maxDegrees
  ))
  ...
  pts = derivePoints(data, lng, lat, missing(lng), missing(lat), "addPieChartMarkers")
  invokeMethod(map, data, 'addPieChartMarkers', pts$lat, pts$lng,
               layerId, group, options, clusterOptions, clusterId, popup) %>%
    expandLimits(pts$lat, pts$lng)
}
...
markerDvfDependencies = function() {
  list(
    htmltools::htmlDependency(
      'leaflet-dvf',
      '0.2.3',
      system.file('htmlwidgets/plugins/leaflet-dvf', package = 'leaflet'),
      script = c('leaflet-dvf.js', 'leaflet-dvf.markers.js'),
      stylesheet = 'leaflet-dvf.css'
    )
  )
}
